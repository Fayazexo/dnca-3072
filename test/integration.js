...
async function runIntegrationTests() {
    ...
    try {
        ...
        const encrypted = await dnca.encrypt(testMessage, encryptionKey);
        ...
        try {
            ...
            test('Tampered data detection', error.message.includes('Authentication failed'));
        }
        
        // Test cross-key isolation
        const otherMaster = await dnca.generateMasterKey();
        const otherEncKey = await dnca.generateEncryptionKey(otherMaster);
        const otherDecKey = await dnca.generateDecryptionKey(otherMaster, otherEncKey);
